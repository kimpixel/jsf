...
export const withContextToExpandPanelProps = (
  ...
  let getTitle = (properties, childData) => {
    let title;
    switch (true) {
  ...
  let getDynamicChildLabel = (schema, childData) => {
    let label = '';
    if(schema.oneOf) {

      const subSchema = schema.oneOf.find((oneOf) =>
        oneOf.properties.type?.enum.includes(childData.type)
      );

      if(uischema.oneOfLabels?.hasOwnProperty(childData.type)) {
        label = `[${uischema.oneOfLabels[childData.type]}] - `;
      } else {

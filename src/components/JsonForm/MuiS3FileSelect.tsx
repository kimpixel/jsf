...
export const MuiS3FileSelect = React.memo((props: EnumCellProps & WithClassname & TranslateProps) => {
  const {
    data,
    className,
    id,
      ...
      renderValue={(value) =>
          <Box sx={{display: 'flex', gap: 1, alignItems: 'center'}}>
            <Avatar
                ...
                alt="S3"
                sx={{bgcolor: '#000'}}
                src={options.find(o => o.value.key == value)?.value.file}
            />
    ...
    >
      {[<MenuItem value={''} key='jsonforms.enum.none'><em>{noneOptionLabel}</em></MenuItem>].concat(
        options.map(optionValue => (
          <MenuItem sx={{gap: '15px'}} value={optionValue.value.key} key={optionValue.value.key}>
            <Avatar variant="rounded" alt="S3" sx={{ bgcolor: '#000' }} src={ optionValue.value.file} /> {optionValue.label}
          </MenuItem>
        ))
      )}
    </Select>
